...
os.mkdir(ROOT_DIR + 'logfiles/' + TIMESTAMP)
print((ROOT_DIR + 'logfiles/' + TIMESTAMP))
DEBUG_FILE = open(ROOT_DIR + 'logfiles/' + TIMESTAMP + '/debug.txt', 'w')
INFO_FILE = open(ROOT_DIR + 'logfiles/' + TIMESTAMP + '/info.txt', 'w')
os.mkdir(ROOT_DIR + 'logfiles/' + TIMESTAMP + '/test_outputs/')
...
class ErrorLogger(object):
    def __init__(self):
        ...
        self.log = open(ROOT_DIR + 'logfiles/' + TIMESTAMP + '/stderr.txt', 'a')
...
def define_network(X):
    ...
    if CONTINUE_TRAINING or TEST_ONLY:
        saved_params_npz = np.load(ROOT_DIR + 'logfiles/' + TRAINED_NETWORK_TIMESTAMP + '/network_parameters_epoch_' + str(LOAD_EPOCH) + '.npz')
...
def train_model(learning_rate, data, num_epochs=100):
    ...
    plot_module.plot_losses(LossTrain, train_samples_per_epoch, LossTest, len(indices_test), ROOT_DIR, TIMESTAMP)
...
def save_network(output, filename_prolongation=''):
    ...
    np.savez(ROOT_DIR + 'logfiles/' + TIMESTAMP + '/network_parameters' + filename_prolongation + '.npz', final_params)
